...
public class PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        ...
        int index = 0;
        while (index < myPrefixSize && index < v.getWord().length() && index < w.getWord().length()) {
            if (v.getWord().charAt(index) != w.getWord().charAt(index)) {
                return v.getWord().charAt(index) - w.getWord().charAt(index);
            }
            index++;
        }
        ...
        if (index == v.getWord().length() && index == w.getWord().length()) {
            return 0;
        } else if (index == v.getWord().length()) {
            return -1;
        } else {
            return 1;
        }
